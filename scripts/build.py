...
def references():
    ...
    body=section('The ideas, in their own words.',f'<p class="narrow">Read the current planning document, trace an idea to the original presentations, or follow a connected public project. This library contains {len(docs)} supplied documents and all 15 supplied website links. Original files are preserved without rewriting their contents.</p><div class="reading-links"><a href="#documents">Documents</a><a href="#websites">Websites</a><a href="#provenance">How to read the sources</a></div>','light')
    ...
    for doc in docs:
        ...
        body+=f'<article class="reference-item" data-search="{e(title+" "+doc.get("category",""))}"><div><h3>{e(title)}</h3><p>{e(doc.get("category","Original reference"))} · {ext} · {size_text}</p></div>{f"<a href={e(href)} download>Download {ext} ↓</a>" if href else "<span>Source unavailable</span>"}</article>'
...
def generic(chapter):
    ...
    for i,s in enumerate(chapter['sections']):
        ...
        if i==0 and not s.get('image'):result+='<div>'+image(chapter['hero'],'AI-generated concept illustration for '+chapter['title'],'wide-art')+'<p class="wrap image-caption">AI-generated concept illustration.</p></div>'
...
ORDER=['index','vision','community','leadership','ai-queens','ventures','catalogue','shared-intelligence','care','capital','brisbane','global','join','references','licence']
META={
'index':('Home','Ambition deserves<br><em>a bigger stage.</em>','A new generation of women leading enterprise, shaping investment and building a more abundant world. Beginning in Brisbane. Open to possibility.','home-hero'),
...
def render(slug,meta,body,pages):
    ...
    nav=''.join(f'<a href="{link(s)}" {"aria-current=page" if s==slug else ""}>{META[s][0]}</a>' for s in pages)
...
def main():
    ...
    (ROOT/'.nojekyll').touch()
    (ROOT/'sitemap.xml').write_text('<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">'+''.join(f'<url><loc>{BASE+("" if s=="index" else s+".html")}</loc><lastmod>2026-09-23</lastmod></url>' for s in pages)+'</urlset>',encoding='utf-8')
