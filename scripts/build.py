...
<main id="main"><section class="hero {'home' if slug=='index' else 'detail-hero' if slug.startswith(('queen-','startup-','category-')) else ''}">{image(art,'AI-generated concept artwork for '+title,'hero-bg',True)}<div class="wrap"><div class="hero-copy">{f'<div class="hero-crumb"><a href="index.html">500 Queens</a> / {e(title)}</div>' if slug!='index' else ''}<h1>{heading}</h1><p>{dek}</p>{actions}</div></div><span class="hero-meta">GenAI concept artwork</span></section>{body}</main>
...
def main():
    ...
    (ROOT/'.nojekyll').touch()
    sitemap_pages=pages+(list(network['pages']) if network else [])
