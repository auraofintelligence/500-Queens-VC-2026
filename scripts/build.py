...
def generic(chapter):
    ...
    for i,s in enumerate(chapter['sections']):
        ...
        if s.get('photographs'):
            content+='<div class="about-photographs">'+''.join(f'<figure><img src="{e(p["src"])}" alt="{e(p["alt"])}" width="{p["width"]}" height="{p["height"]}" loading="lazy" decoding="async"><figcaption>{e(p["caption"])}</figcaption></figure>' for p in s['photographs'])+'</div>'
        block=section(s['heading'],content,'light' if i%3==0 else ('plum' if i%3==2 else ''))
        if s.get('id'):block=block.replace('<section ',f'<section id="{e(s["id"])}" ',1)
        result+=block
...
ORDER=['index','vision','designing-equality','community','leadership','ai-queens','ventures','catalogue','network','shared-intelligence','care','capital','brisbane','global','join','site-map','references','licence']
META={
...
def main():
    ...
    if network:
        ...
        council_preview=section('A Council of archetypes.<br><em>A world of possibility.</em>','<p>Meet 24 ageless AI Queens, each a distinct expression of cultural identity, mastered intelligence and vitality. Their original archetypes inspire individual clothing, colours and forms, from athletic to voluptuous. Follow each Queen into her domain, learning sessions and connected enterprise ideas.</p><div class="queen-feature-strip">'+''.join(network_builder.queen_card(q) for q in featured)+'</div><div class="actions"><a class="button" href="ai-queens.html">Meet all 24 Queens →</a><a class="button secondary" href="network.html">Build my pathway →</a></div>')
    ...
    if network:
        content['site-map']=build_site_map(ROOT,helpers)
        META['site-map']=('Visual site map','See the whole world.<br><em>Choose your own path.</em>','A visual guide to the Queens, enterprise ideas, planning tools and original sources.','library-hero')
