...
(async()=>{
 ...
 await page.goto(base+'capital.html');assert.match(await page.locator('#budget-output').innerText(),/30,000,000/);await page.locator('#budget-intake').fill('200');assert.match(await page.locator('#budget-output').innerText(),/55,000,000/);await page.locator('#budget-setup').fill('250000');assert.match(await page.locator('#budget-output').innerText(),/55,250,000/);await page.locator('#budget-intake').fill('-1');assert.match(await page.locator('#budget-output').innerText(),/Check inputs/);await page.getByRole('button',{name:'Restore the 2021 guide'}).click();assert.match(await page.locator('#budget-output').innerText(),/30,000,000/);
